...
__on_start() -> (
	...
	scoreboard_display('list', 'scbd.health');
	scoreboard_display('belowName', 'scbd.health');
...
update_scbds(player) -> (
	if (query(player, 'player_type') == 'fake',
		for(global_scoreboards,
			if(_ != 'scbd.health',
				run(str('scoreboard players reset %s %s', player, _));
			);
		);
	,
